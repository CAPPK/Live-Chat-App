...
def privatemessage():
    ...
    if request.method == 'POST':
        ...
        match = list(query.fetch())
        if match:
            # query database for an array containing previous chat Messages
            messages = []
            return render_template("/privatemessage/privateconvo.html", code=302)
    return render_template("/privatemessage/searchUser.html", code=302)
